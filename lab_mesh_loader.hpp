...
namespace lab {
	...
	void _loadObjFile(const std::string &filename, std::vector<VertexFormat> &vertices, std::vector<unsigned int> &indices){
		...
		// translatez obiect in origine
		float x_max = INT_MIN, x_min = INT_MAX , y_max = INT_MIN, y_min = INT_MAX, z_max = INT_MIN, z_min = INT_MAX;
		for (int i = 0; i < vertices.size(); ++i) {
			if (vertices[i].position_x > x_max)
				x_max = vertices[i].position_x;
			if (vertices[i].position_x < x_min)
				x_min = vertices[i].position_x;
			if (vertices[i].position_y > y_max)
				y_max = vertices[i].position_y;
			if (vertices[i].position_y < y_min)
				y_min = vertices[i].position_y;
			if (vertices[i].position_z > z_max)
				z_max = vertices[i].position_z;
			if (vertices[i].position_z < z_min)
				z_min = vertices[i].position_z;
		}
		...
		z_dif = (z_max + z_min) / 2;
		for (int i = 0; i < vertices.size(); ++i) {
			vertices[i].position_x -= x_dif;
			vertices[i].position_y -= y_dif;
			vertices[i].position_z -= z_dif;
		}
	}
